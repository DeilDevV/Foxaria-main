...
public final class RegionRepository {
    ...
    /**
     * Кэш состава приватов: regionId → множество участников (включая владельца).
     *
     * Зачем: isMember() дергался из событий движения, боя и взаимодействия
     * с блоками через .join() — то есть КАЖДЫЙ шаг игрока внутри привата
     * блокировал главный поток SQL-запросом. На людном сервере это заметные
     * просадки TPS. Состав меняется редко, поэтому держим его в памяти.
     */
    private final ConcurrentMap<Integer, Set<UUID>> memberCache = new ConcurrentHashMap<>();
    ...
    public CompletableFuture<Void> addMember(int regionId, UUID uuid, String role) {
        ...
        }).thenRun(() -> invalidateMembers(regionId));
    }

    public CompletableFuture<Boolean> isMember(int regionId, UUID uuid) {
        Set<UUID> cached = memberCache.get(regionId);
        if (cached != null) {
            return CompletableFuture.completedFuture(cached.contains(uuid));
        }
        return memberUuids(regionId).thenApply(members -> members.contains(uuid));
    }

    /** Мгновенная проверка по кэшу, без обращения к БД (для «горячих» событий). */
    public boolean isMemberCached(int regionId, UUID uuid) {
        Set<UUID> cached = memberCache.get(regionId);
        if (cached != null) {
            return cached.contains(uuid);
        }
        // Кэша ещё нет — прогреваем в фоне, сейчас считаем «не участник».
        memberUuids(regionId);
        return false;
    }

    /** Загружен ли состав региона в кэш. */
    public boolean hasMemberCache(int regionId) {
        return memberCache.containsKey(regionId);
    }

    public void invalidateMembers(int regionId) {
        memberCache.remove(regionId);
    }
    ...
    public CompletableFuture<Set<UUID>> memberUuids(int regionId) {
        ...
        return database.query(connection -> {
            ...
            Set<UUID> snapshot = Set.copyOf(loaded);
            memberCache.put(regionId, snapshot);
            return snapshot;
        });
    }
