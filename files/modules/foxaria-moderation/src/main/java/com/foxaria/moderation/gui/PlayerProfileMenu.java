...
public final class PlayerProfileMenu extends BaseMenu {
    ...
    protected void draw(Player viewer) {
        ...
        setItem(29, MenuItems.item(
            ...
            "&6&lИнвентарь &8(только просмотр)",
            "&7Снимок рюкзака игрока для проверки.",
            "&8Забрать предметы нельзя —",
            "&8это копия, а не живой инвентарь."
        ), click -> {
            Player targetPlayer = Bukkit.getPlayer(targetUuid);
            if (targetPlayer == null) {
            ...
            // Раньше открывался ЖИВОЙ инвентарь: модератор мог забрать вещи.
            // Теперь — неизменяемый снимок (см. ReadOnlyInventoryView).
            ReadOnlyInventoryView.open(viewer, targetPlayer, false);
        });
