...
public final class TeleportService {
    ...
    /**
     * Блоки, в которых игрок не должен оказаться (или стоять на них).
     * Раньше проверка была через isPassable(), но он возвращает true
     * и для ВОДЫ, и для ЛАВЫ — игрока могло закинуть в лавовое озеро.
     */
    private static final java.util.Set<Material> HAZARD_BLOCKS = java.util.Set.of(
        ...
        Material.END_GATEWAY, Material.MAGMA_BLOCK, Material.CAMPFIRE,
        Material.SOUL_CAMPFIRE, Material.POINTED_DRIPSTONE, Material.BUBBLE_COLUMN
    );

    /** Материалы-маркеры чужих построек: рядом с ними RTP не высаживает. */
    private static final java.util.List<String> STRUCTURE_MARKERS = java.util.List.of(
        "PLANKS", "CHEST", "FURNACE", "CRAFTING_TABLE", "GLASS", "CONCRETE",
        "WOOL", "TERRACOTTA", "BRICKS", "IRON_BLOCK", "GOLD_BLOCK", "DIAMOND_BLOCK",
        "NETHERITE_BLOCK", "HOPPER", "ANVIL", "BEACON", "ENCHANTING_TABLE",
        "_BED", "_DOOR", "LADDER", "RAIL", "TORCH", "BARREL", "SHULKER_BOX",
        "SCAFFOLDING", "GLAZED", "COPPER_BLOCK", "SPAWNER", "BOOKSHELF"
    );

    // Кэш рефлексии для проверки приватов: раньше getMethod() дёргался
    // для КАЖДОГО региона на КАЖДУЮ попытку — тысячи вызовов на один RTP.
    ...
    public void startRtpPool() {
        ...
        // Пул прогревается в фоне и медленно: именно он избавляет игрока от
        // ожидания при RTP. Частый тик тут не нужен — важнее не грузить сервер.
        long period = Math.max(40L, configs.main().getLong("rtp.pool.refill-period-ticks", 100L));
    ...
    private void fillPool(World world, int targetSize) {
        ...
        if (!poolFilling.compareAndSet(false, true)) {
            return;
        }
        findSafeRandomLocationBatched(world, RtpReason.POOL).whenComplete((opt, error) -> {
            try {
                if (error == null && opt != null) {
                    opt.ifPresent(rtpPool::add);
                }
            } finally {
                poolFilling.set(false);
                // Раньше здесь был мгновенный рекурсивный вызов: на старте
                // сервер пытался набрать весь пул за раз и уходил в шторм
                // генерации чанков — отсюда фризы и вылеты игроков.
                if (rtpPool.size() < targetSize) {
                    long delay = Math.max(20L, configs.main().getLong("rtp.pool.fill-delay-ticks", 60L));
                    plugin.getServer().getScheduler().runTaskLater(plugin,
                        () -> fillPool(world, targetSize), delay);
                }
            }
        });
    }
    ...
    private CompletableFuture<Optional<Location>> findSafeRandomLocationBatched(World world, RtpReason reason) {
        ...
        if (minDistance >= radius) {
            minDistance = Math.max(0, radius / 4);
        }
        // Раньше значение из конфига принудительно поднималось до 100 попыток
        // (Math.max(attempts, 100)) — на новой карте это до сотни генераций
        // чанков подряд, отсюда фризы, вылеты по таймауту и тормоза прогрузки.
        ...
        // Сколько НОВЫХ чанков разрешено сгенерировать за один поиск.
        // Остальные попытки используют уже существующие чанки — это главный
        // тормоз RTP, и именно он раньше вешал сервер.
        ...
        SearchContext ctx = new SearchContext(attempts, radius, minDistance, avoidRegions,
            minSurfaceY, maxSurfaceScan, new java.util.concurrent.atomic.AtomicInteger(generateBudget));

        // Страховка: если чанк не догрузился и колбэк не пришёл, поиск раньше
        // висел вечно, а игрок оставался с титром «Ищем безопасную точку…».
    ...
    /**
     * Асинхронный поиск точки RTP.
     * Чанки грузятся через Paper getChunkAtAsync (воркер-треды), проверка
     * блоков — в колбэке на главном потоке. Рекурсия «хвостовая» через
     * CompletableFuture, стек не растёт.
     */
    private void findSafeAsync(World world, int attempt, SearchContext ctx,
        ...
        // Учитываем rtp.min-distance: раньше параметр лежал в конфиге,
        // но нигде не читался — игрока могло высадить вплотную к 0,0.
        ...
        if (allowGenerate) {
            ctx.generateBudget().decrementAndGet();
        }

        world.getChunkAtAsync(x >> 4, z >> 4, allowGenerate)
            .exceptionally(ex -> null)
            .thenAccept(chunk -> {
                if (out.isDone()) {
                    return;
                }
                // Чтение блоков и getChunkAtAsync допустимы только с главного потока.
                // При ошибке загрузки колбэк может прийти на чужом треде, поэтому
                // на всякий случай возвращаемся в основной поток.
                Runnable step = () -> {
                    if (out.isDone()) {
                        return;
                    }
                    // chunk == null: чанк не сгенерирован и генерация запрещена — идём дальше.
                    if (chunk != null) {
                        Optional<Location> safe = findSafeSurface(world, x, z, ctx.minSurfaceY(), ctx.maxSurfaceScan());
                        if (safe.isPresent()) {
                            Location candidate = safe.get();
                            boolean nearRegion = ctx.avoidRegions() > 0
                                && isNearAnyRegionReflective(candidate, ctx.avoidRegions());
                            if (!nearRegion && !isNearPlayerStructure(candidate)) {
                ...
                if (plugin.getServer().isPrimaryThread()) {
                    ...
                    plugin.getServer().getScheduler().runTask(plugin, step);
                }
            });
    }
    ...
    private Optional<Location> findSafeSurface(World world, int x, int z, int minSurfaceY, int scanDown) {
        ...
        for (int tries = 0; tries < scanDown && y >= minY; tries++, y--) {
            ...
            // Ноги и голова — свободное безопасное место (не жидкость, не огонь).
            if (!isFreeSpace(feet) || !isFreeSpace(head)) {
                continue;
            }
            // Под ногами — твёрдая земля, а не листва, лава или магма.
            if (!isSafeGround(below)) {
                continue;
            }
            // Опасность вплотную (лава/огонь сбоку или потолок из лавы).
            if (hasHazardAround(world, x, y, z)) {
                continue;
            }
    ...
    /** Свободное место для игрока: воздух или безопасная «проходимая» трава. */
    private boolean isFreeSpace(org.bukkit.block.Block block) {
        ...
        // isPassable() == true у воды и лавы, поэтому фильтруем явно.
        return !HAZARD_BLOCKS.contains(type) && !block.isLiquid();
    }

    /** Твёрдая поверхность под ногами: не «воздух», не листва, не опасный блок. */
    private boolean isSafeGround(org.bukkit.block.Block below) {
        if (!below.isSolid() || below.isLiquid()) {
            return false;
        }
        Material type = below.getType();
        if (HAZARD_BLOCKS.contains(type)) {
            return false;
        }
    ...
    /** Лава/огонь вплотную к точке приземления. */
    private boolean hasHazardAround(World world, int x, int y, int z) {
        ...
        for (int[] off : offsets) {
            ...
            if (side == Material.LAVA || side == Material.FIRE || side == Material.SOUL_FIRE) {
                return true;
            }
        }
    ...
    /**
     * Рядом ли чужая постройка. Дешёвая выборка по сетке с шагом,
     * читаются только уже загруженные чанки — без подгрузки соседей.
     */
    private boolean isNearPlayerStructure(Location candidate) {
        if (!configs.main().getBoolean("rtp.avoid-structures.enabled", true)) {
            return false;
        }
        ...
        for (int dx = -radius; dx <= radius; dx += step) {
            for (int dz = -radius; dz <= radius; dz += step) {
                ...
                for (int dy = -2; dy <= 4; dy += 2) {
                    Material type = world.getBlockAt(wx, baseY + dy, wz).getType();
                    if (type.isAir()) {
                        continue;
                    }
                    String name = type.name();
    ...
    private record SearchContext(
        ...
        int minSurfaceY,
        int maxSurfaceScan,
        java.util.concurrent.atomic.AtomicInteger generateBudget
    ) {
    }
    ...
    private boolean isNearAnyRegionReflective(Location loc, int minDistance) {
        ...
        try {
            ...
            // Методы кэшируем: раньше getMethod() вызывался для каждого
            // региона на каждую попытку RTP — тысячи рефлективных вызовов.
            ...
            for (Object r : regions) {
                if (r == null) continue;
                String rw = (String) regionWorldMethod.invoke(r);
                if (!world.equals(rw)) continue;
                int cx = (int) regionCenterXMethod.invoke(r);
                int cz = (int) regionCenterZMethod.invoke(r);
                int half = (int) regionHalfSizeMethod.invoke(r);
