...
public final class FirstJoinSpawnLocationListener implements Listener {
    ...
    public void onAsyncSpawn(AsyncPlayerSpawnLocationEvent event) {
        ...
        // Точка берётся из прогретого пула — никакой генерации чанков в событии
        // логина (именно она раньше подвешивала вход и выкидывала по таймауту).
        // Если пул пуст, отметку НЕ ставим: заброс сделает запасной листенер
        // на PlayerJoinEvent, и стартовый спавн не потеряется.
        teleport.consumePreparedRtpLocation(world).ifPresent(loc -> {
            event.setSpawnLocation(loc);
            firstJoin.markFirstJoinThisLogin(uuid);
