...
public final class ItemTemplateChatListener implements Listener {
    ...
    private void handleSync(Player player, ItemTemplateWorkbenchSession.State state, ItemTemplateWorkbenchSession.ChatPrompt prompt, String text) {
        ...
        if (prompt == ItemTemplateWorkbenchSession.ChatPrompt.DISPLAY_NAME) {
            ...
            // Строка уходит в PDC, а описание пересобирается целиком —
            // так дубли строк невозможны даже при повторных правках.
            com.foxaria.itemtemplates.ability.AbilityCodec.addUserLoreLine(plugin, stack, text);
        }
        com.foxaria.itemtemplates.ability.AbilityLoreRenderer.apply(plugin, stack);
        ...
        // Возвращаем игрока в кузницу: раньше после ввода в чат
        // приходилось открывать меню заново вручную.
        player.performCommand("itemtemplate forge");
    }
}
