...
public final class ItemTemplateCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            messages.send(sender, "itemtemplate.usage", "&e/itpl gui | edit | save | give | clearlore | glint on|off | … &7(см. справку в чате)");
            return true;
        }
        ...
        switch (sub) {
            case "gui" -> {
                ...
                menuManager.open(player, new ItemTemplateWorkbenchMenu(plugin, menuManager, templates, messages, editorConfig, ItemTemplateWorkbenchMenu.Panel.HOME));
                return true;
            }
            case "edit" -> {
                if (!(sender instanceof Player player)) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            return List.of("gui", "edit", "save", "delete", "give", "list", "reload", "export", "clearlore", "glint").stream()
