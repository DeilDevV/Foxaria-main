...
# Foxaria Lobby backend start (Paper)
...
if (!(Test-Path -Path $Jar)) {
  Write-Host "[ERROR] Не найден '$Jar' в папке servers\lobby"
...
if ($LASTEXITCODE -ne 0) {
  ...
  Write-Host "Установи Java 21+ и укажи JAVA_HOME, либо положи JDK в servers\lobby\jdk"
  Read-Host "Нажми Enter чтобы выйти" | Out-Null
  exit 1
}
