...
$FoxariaRoot = Split-Path (Split-Path $TestServerRoot -Parent) -Parent
...
$jar = Get-ChildItem "$FoxariaRoot\.gradle-build\foxaria-bootstrap\libs\foxaria-bootstrap-*-SNAPSHOT.jar" |
    Where-Object { $_.Name -notmatch "sources" } |
    Select-Object -First 1
if (-not $jar) {
    Write-Host "JAR не найден в .gradle-build\foxaria-bootstrap\libs\" -ForegroundColor Red
    exit 1
}
...
$modSrc = "$FoxariaRoot\modules\foxaria-bootstrap\src\main\resources\modules"
...
$questsTier = "$FoxariaRoot\config\gameplay\quests.yml"
if (Test-Path $questsTier) {
    Copy-Item -LiteralPath $questsTier -Destination (Join-Path $modDest "quests.yml") -Force
    Write-Host "quests.yml (ступени): из config\gameplay\quests.yml" -ForegroundColor Green
}
